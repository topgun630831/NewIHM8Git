...
/*
 * C:/CodeInspection/DisplayUnit/STemWin/Board/ts/ads7843_ts.c
 */
/*     7 */ Matrix matrix ;
/*     8 */ Coordinate display ;
/*     9 */ Coordinate ScreenSample[3];
/*    10 */ Coordinate DisplaySample[3] = {
/*    11 */    { 50, 45 },
/*    12 */    { 290, 100},
/*    13 */    { 160,210}
/*    14 */ };
/*    35 */ void DelayUS(uint32_t cnt)
/*    36 */ {
/*    37 */    uint32_t i;
/*    38 */    i = cnt * 4;
/*    39 */    while(i--);
/*    40 */ }
/*    43 */ void WR_CMD (unsigned char cmd)
/*    44 */ {
/*    45 */    HAL_StatusTypeDef nErrStats;
/*    47 */    nErrStats = HAL_SPI_Transmit(&hspi3, &cmd, 1, 1000);
/*    48 */    if(nErrStats != HAL_OK) printf("wr err : 0x%x\n\r", nErrStats);
/*    49 */ }
/*    51 */ int RD_AD(void)
/*    52 */ {
/*    53 */    unsigned char buf[2];
/*    54 */    int value;
/*    56 */    HAL_SPI_Receive(&hspi3, buf, 2, 1000);
/*    57 */    value = (unsigned short)((buf[0] << 8) + buf[1]) >> 3;
/*    59 */    return value;
/*    60 */ }
/*    63 */ int Read_X(void)
/*    64 */ {
/*    65 */    int i;
/*    68 */    HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0000U)), ((unsigned short)0x8000), GPIO_PIN_RESET);
/*    70 */    delayUs(1);
/*    71 */    WR_CMD(0x90);
/*    72 */    delayUs(1);
/*    73 */    i=RD_AD();
/*    76 */    HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0000U)), ((unsigned short)0x8000), GPIO_PIN_SET);
/*    78 */    return i;
/*    79 */ }
/*    81 */ int Read_Y(void)
/*    82 */ {
/*    83 */    int i;
/*    86 */    HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0000U)), ((unsigned short)0x8000), GPIO_PIN_RESET);
/*    88 */    delayUs(1);
/*    89 */    WR_CMD(0xd0);
/*    90 */    delayUs(1);
/*    91 */    i=RD_AD();
/*    94 */    HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0000U)), ((unsigned short)0x8000), GPIO_PIN_SET);
/*    96 */    return i;
/*    97 */ }
/*    99 */ void TP_GetAdXY(int *x, int *y)
/*   100 */ {
/*   101 */    int adx, ady;
/*   102 */    adx=Read_X();
/*   103 */    delayUs(1);
...
/*   106 */    *y=ady;
/*   107 */ }
/*   109 */ void TP_DrawPoint(unsigned short Xpos,unsigned short Ypos)
/*   110 */ {
/*   117 */    DrawCross(Xpos, Ypos);
/*   119 */ }
/*   122 */ void DrawCross(unsigned short Xpos,unsigned short Ypos)
/*   123 */ {
/*   124 */    Draw_Line(Xpos-15, Ypos, Xpos-2, Ypos, 0xffffff);
/*   125 */    Draw_Line(Xpos+2, Ypos, Xpos+15, Ypos, 0xffffff);
/*   126 */    Draw_Line(Xpos, Ypos-15, Xpos, Ypos-2, 0xffffff);
/*   127 */    Draw_Line(Xpos, Ypos+2, Xpos, Ypos+15, 0xffffff);
/*   142 */ }
/*   145 */ Coordinate *Read_Ads7843(void)
/*   146 */ {
/*   147 */    static Coordinate screen;
/*   148 */    int m0,m1,m2,TP_X[1],TP_Y[1],temp[3];
/*   149 */    unsigned char count=0;
/*   150 */    int buffer[2][9]={{0},{0}};
/*   152 */    do
/*   153 */    {
/*   154 */       TP_GetAdXY(TP_X, TP_Y);
/*   155 */       buffer[0][count] = TP_X[0];
/*   156 */       buffer[1][count] = TP_Y[0];
/*   157 */       count++;
/*   158 */    }
/*   159 */    while( !HAL_GPIO_ReadPin(((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0400U)), ((unsigned short)0x0010)) && (count<9));
/*   162 */    if(count==9)
/*   163 */    {
/*   165 */       temp[0]=(buffer[0][0]+buffer[0][1]+buffer[0][2])/3;
/*   166 */       temp[1]=(buffer[0][3]+buffer[0][4]+buffer[0][5])/3;
/*   167 */       temp[2]=(buffer[0][6]+buffer[0][7]+buffer[0][8])/3;
/*   169 */       m0=temp[0]-temp[1];
/*   170 */       m1=temp[1]-temp[2];
/*   171 */       m2=temp[2]-temp[0];
/*   173 */       m0=m0>0?m0:(-m0);
/*   174 */       m1=m1>0?m1:(-m1);
/*   175 */       m2=m2>0?m2:(-m2);
/*   177 */       if( m0>2 && m1>2 && m2>2 )
/*   178 */       {
/*   179 */          return 0;
/*   180 */       }
/*   182 */       if(m0<m1)
/*   183 */       {
/*   184 */          if(m2<m0)
/*   185 */          screen.x=(temp[0]+temp[2])/2;
/*   186 */          else
/*   187 */          screen.x=(temp[0]+temp[1])/2;
/*   188 */       }
/*   189 */       else if(m2<m1)
/*   190 */       screen.x=(temp[0]+temp[2])/2;
/*   191 */       else
/*   192 */       screen.x=(temp[1]+temp[2])/2;
/*   195 */       temp[0]=(buffer[1][0]+buffer[1][1]+buffer[1][2])/3;
/*   196 */       temp[1]=(buffer[1][3]+buffer[1][4]+buffer[1][5])/3;
/*   197 */       temp[2]=(buffer[1][6]+buffer[1][7]+buffer[1][8])/3;
/*   198 */       m0=temp[0]-temp[1];
/*   199 */       m1=temp[1]-temp[2];
/*   200 */       m2=temp[2]-temp[0];
/*   201 */       m0=m0>0?m0:(-m0);
/*   202 */       m1=m1>0?m1:(-m1);
/*   203 */       m2=m2>0?m2:(-m2);
/*   204 */       if(m0>2&&m1>2&&m2>2) return 0;
/*   206 */       if(m0<m1)
/*   207 */       {
/*   208 */          if(m2<m0)
/*   209 */          screen.y=(temp[0]+temp[2])/2;
/*   210 */          else
/*   211 */          screen.y=(temp[0]+temp[1])/2;
/*   212 */       }
/*   213 */       else if(m2<m1)
/*   214 */       screen.y=(temp[0]+temp[2])/2;
/*   215 */       else
/*   216 */       screen.y=(temp[1]+temp[2])/2;
/*   218 */       return &screen;
/*   219 */    }
/*   220 */    return 0;
/*   221 */ }
/*   224 */ FunctionalState setCalibrationMatrix( Coordinate * displayPtr, Coordinate * screenPtr, Matrix * matrixPtr)
/*   225 */ {
/*   227 */    FunctionalState retTHRESHOLD = ENABLE ;
/*   229 */    matrixPtr->Divider = ((screenPtr[0].x - screenPtr[2].x) * (screenPtr[1].y - screenPtr[2].y)) - ((screenPtr[1].x - screenPtr[2].x) * (screenPtr[0].y - screenPtr[2].y)) ;
/*   230 */    if( matrixPtr->Divider == 0 )
/*   231 */    {
/*   232 */       retTHRESHOLD = DISABLE;
/*   233 */    }
/*   234 */    else
/*   235 */    {
/*   236 */       matrixPtr->An = ((displayPtr[0].x - displayPtr[2].x) * (screenPtr[1].y - screenPtr[2].y)) - ((displayPtr[1].x - displayPtr[2].x) * (screenPtr[0].y - screenPtr[2].y)) ;
/*   237 */       matrixPtr->Bn = ((screenPtr[0].x - screenPtr[2].x) * (displayPtr[1].x - displayPtr[2].x)) - ((displayPtr[0].x - displayPtr[2].x) * (screenPtr[1].x - screenPtr[2].x)) ;
/*   238 */       matrixPtr->Cn = (screenPtr[2].x * displayPtr[1].x - screenPtr[1].x * displayPtr[2].x) * screenPtr[0].y + (screenPtr[0].x * displayPtr[2].x - screenPtr[2].x * displayPtr[0].x) * screenPtr[1].y + (screenPtr[1].x * displayPtr[0].x - screenPtr[0].x * displayPtr[1].x) * screenPtr[2].y ;
/*   239 */       matrixPtr->Dn = ((displayPtr[0].y - displayPtr[2].y) * (screenPtr[1].y - screenPtr[2].y)) - ((displayPtr[1].y - displayPtr[2].y) * (screenPtr[0].y - screenPtr[2].y)) ;
/*   240 */       matrixPtr->En = ((screenPtr[0].x - screenPtr[2].x) * (displayPtr[1].y - displayPtr[2].y)) - ((displayPtr[0].y - displayPtr[2].y) * (screenPtr[1].x - screenPtr[2].x)) ;
/*   241 */       matrixPtr->Fn = (screenPtr[2].x * displayPtr[1].y - screenPtr[1].x * displayPtr[2].y) * screenPtr[0].y + (screenPtr[0].x * displayPtr[2].y - screenPtr[2].x * displayPtr[0].y) * screenPtr[1].y + (screenPtr[1].x * displayPtr[0].y - screenPtr[0].x * displayPtr[1].y) * screenPtr[2].y ;
/*   242 */    }
/*   243 */    return( retTHRESHOLD ) ;
/*   244 */ }
/*   246 */ FunctionalState getDisplayPoint(Coordinate * displayPtr, Coordinate * screenPtr, Matrix * matrixPtr )
/*   247 */ {
/*   248 */    FunctionalState retTHRESHOLD = ENABLE;
/*   251 */    if( matrixPtr->Divider != 0 )
/*   252 */    {
/*   254 */       displayPtr->x = (unsigned short)(( (matrixPtr->An * screenPtr->x) + (matrixPtr->Bn * screenPtr->y) + matrixPtr->Cn ) / matrixPtr->Divider);
/*   256 */       displayPtr->y = (unsigned short)(( (matrixPtr->Dn * screenPtr->x) + (matrixPtr->En * screenPtr->y) + matrixPtr->Fn ) / matrixPtr->Divider);
/*   257 */    }
/*   258 */    else
/*   259 */    {
/*   260 */       retTHRESHOLD = DISABLE;
/*   261 */    }
/*   262 */    return(retTHRESHOLD);
/*   263 */ }
/*   265 */ void TouchPanel_Calibrate(void)
/*   266 */ {
/*   267 */    unsigned char i;
/*   268 */    Coordinate * Ptr;
/*   269 */    char txtbuf[60]={0};
/*   271 */    LCD_Clear(0, 0, 320, 480, 0x000000);
/*   273 */    for(i=0;i<3;i++)
/*   274 */    {
/*   275 */       Display_String(44, 10, 0xffffff, "Touch crosshair to calibrate");
/*   276 */       HAL_Delay(300);
/*   277 */       DrawCross(DisplaySample[i].x,DisplaySample[i].y);
/*   278 */       do
/*   279 */       {
/*   280 */          Ptr = Read_Ads7843();
/*   281 */       }
/*   282 */       while( Ptr == (void*)0 );
/*   284 */       ScreenSample[i].x= Ptr->x;
/*   285 */       ScreenSample[i].y= Ptr->y;
/*   287 */       sprintf(txtbuf, "%d:X:%d, Y:%d", i, ScreenSample[i].x, ScreenSample[i].y);
/*   288 */       Display_String(44, (30+(i*10)), 0xffffff, (unsigned char*)txtbuf );
/*   289 */    }
/*   291 */    HAL_Delay(3000);
/*   293 */    if(setCalibrationMatrix( &DisplaySample[0],&ScreenSample[0],&matrix ) == DISABLE)
/*   294 */    {
/*   295 */       LCD_Clear(0, 0, 320, 480, 0xff0000);
/*   296 */    }
/*   297 */    else
/*   298 */    {
/*   299 */       LCD_Clear(0, 0, 320, 480, 0x000000);
/*   300 */    }
/*   302 */ }
/*   305 */ unsigned short tempx=0, tempy=0;
/*   306 */ void TS_Get_Coordinate_Display()
/*   307 */ {
/*   308 */    char temp[20]={0};
/*   310 */    getDisplayPoint(&display, Read_Ads7843(), &matrix );
/*   311 */    TP_DrawPoint(display.x,display.y);
/*   312 */    if(tempx != display.x || tempy != display.y)
/*   313 */    {
/*   314 */       sprintf(temp,"X:%3d, Y:%3d", tempx, tempy);
/*   315 */       LCD_Clear(10, 10, 100, 100, 0x000000);
/*   316 */       Display_String(10, 10, 0xffff00, (unsigned char *)temp);
/*   317 */       tempx = display.x;
/*   318 */       tempy = display.y;
/*   319 */    }
/*   320 */ }
/*   335 */ unsigned char BSP_TS_DetectTouch()
/*   336 */ {
/*   344 */    return 1;
/*   346 */ }
/*   348 */ void BSP_TS_GetXY(unsigned short *X, unsigned short *Y)
/*   349 */ {
/*   350 */    TP_GetAdXY((int *)X, (int *)Y);
/*   351 */ }
/*   357 */ void BSP_TS_GetState(TS_StateTypeDef* TsState)
/*   358 */ {
/*   359 */    static uint32_t _x = 0, _y = 0;
/*   360 */    unsigned short xDiff, yDiff , x , y, xr, yr;
/*   362 */    TsState->TouchDetected = BSP_TS_DetectTouch();
/*   364 */    if(TsState->TouchDetected)
/*   365 */    {
/*   366 */       BSP_TS_GetXY(&x, &y);
/*   369 */       y -= 360;
/*   372 */       yr = y / 11;
/*   375 */       if(yr <= 0)
/*   376 */       {
...
/*   410 */       {}
/*   412 */       x = xr;
/*   413 */       xDiff = x > _x? (x - _x): (_x - x);
/*   414 */       yDiff = y > _y? (y - _y): (_y - y);
/*   416 */       if (xDiff + yDiff > 5)
/*   417 */       {
/*   418 */          _x = x;
/*   419 */          _y = y;
/*   420 */       }
...
/*   427 */    }
/*   428 */ }
/*   435 */ void BSP_Pointer_Update(void)
/*   436 */ {
/*   437 */    GUI_PID_STATE TS_State;
/*   438 */    static TS_StateTypeDef prev_state;
/*   439 */    TS_StateTypeDef ts;
/*   441 */    unsigned short xDiff, yDiff;
/*   444 */    BSP_TS_GetState(&ts);
/*   446 */    TS_State.Pressed = ts.TouchDetected;
/*   448 */    xDiff = (prev_state.X > ts.X) ? (prev_state.X - ts.X) : (ts.X - prev_state.X);
/*   449 */    yDiff = (prev_state.Y > ts.Y) ? (prev_state.Y - ts.Y) : (ts.Y - prev_state.Y);
/*   451 */    if((prev_state.TouchDetected != ts.TouchDetected )||
/*   452 */        (xDiff > 3 )||
/*   453 */        (yDiff > 3))
/*   454 */    {
/*   455 */       prev_state.TouchDetected = ts.TouchDetected;
/*   457 */       if((ts.X != 0) && (ts.Y != 0))
/*   458 */       {
/*   459 */          prev_state.X = ts.X;
/*   460 */          prev_state.Y = ts.Y;
/*   461 */       }
/*   472 */       {
/*   473 */          TS_State.Layer = 0;
/*   474 */          TS_State.x = prev_state.X;
/*   475 */          TS_State.y = prev_state.Y;
/*   476 */       }
/*   478 */       GUI_TOUCH_StoreStateEx(&TS_State);
/*   479 */    }
/*   480 */ }
/*   486 */ 
